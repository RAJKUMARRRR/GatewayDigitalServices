  ...
  reducer = (state = initialData, action) => {
    switch (action.type) {
      case FETCH_CHAT_START:
        return loadingMessagesStart(state, action);
      case FETCH_CHAT_SUCCESS:
        return loadingMessagesSuccess(state, action);
      case FETCH_CHAT_FAILED:
        return loadingMessagesFailed(state, action);
      case SEND_MEDIA_START:
        return sendingMedia(state, action);
      case SEND_MEDIA_SUCCESS:
        return sendingMediaSuccess(state, action);
      case SEND_MEDIA_FAILED:
        return sendingMediaFailed(state, action);
      case SEND_MESSAGE:
        return sendingMessage(state, action);
      case SEND_MESSAGE_SUCCESS:
        return sendingMessageSuccess(state, action);
      case SEND_MESSAGE_FAILED:
        return sendingMessageFailed(state, action);
    }
    return state;
  };
