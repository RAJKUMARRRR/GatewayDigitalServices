...
const initialData = {
  ...
  reducer = (state = initialData, action) => {
    switch (action.type) {
      case SEND_OTP_START:
        return sendingOTPStart(state, action);
      case SEND_OTP_SUCCESS:
        return sendOTPSuccess(state, action);
      case SEND_OTP_FAILED:
        return sendOTPFailed(state, action);
      case VERIFY_OTP_START:
        return verifyingOTPStart(state, action);
      case VERIFY_OTP_SUCCESS:
        return verifyOTPSuccess(state, action);
      case VERIFY_OTP_FAILED:
        return verifyOTPFailed(state, action);
      ...
      case LOAD_PROFILE_FAILED:
        return loadProfileFailed(state, action);
      case OTP_RECEIVED:
        return otpReceived(state, action);
    }
    return state;
  };
